...
**T1 / T2 timeout** = `(N+2) × 1 µs` where N is the 16-bit loaded value. The +2 is **+1 µs startup delay** (the Φ2 tick that loads N from the latches doesn't decrement) **+1 µs through-zero underflow** (IRQ fires on the `0 → &FFFF` transition, not when the counter first reaches zero). The WDC datasheet's Fig 18 shows `!IRQ` falling at N+1.5 Φ2 cycles — the half-cycle is the opposite-phase IRQ-pin update. See [[timing/via-timers#anatomy-of-the-2]] for the full decomposition + real-hardware verification.

T1 latches survive timeout; in free-run mode the counter reloads from the latches and continues.

**T2** = interval timer (one-shot, fires once per write to T2C-H) or PB6 pulse counter (ACR bit 5 = 1). No PB7 output, no free-run mode.

Both timers can be re-triggered by writing T1C-H / T2C-H again — useful to extend a count before it fires. The re-trigger repeats the +1 µs load tick, so you can't use it to "snap" the count to an exact value mid-flight without accounting for that extra microsecond.
