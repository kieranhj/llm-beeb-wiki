...
sources: [saa5050-references, naug-ch13-video, beebwiki-address-translation, hd6845sp-hitachi-datasheet, bbc-user-guide, master-rm]
...
Per [[sources/master-rm]] Ch E.5, the MOS VDU driver substitutes 3 ASCII codes before writing to MODE 7 screen RAM, so the teletext character generator displays them consistently with non-teletext modes:
...
Per MRM Ch E.5, the following VDU commands behave normally in MODE 7 and 135. **All others are parsed and ignored** (except VDU 25 PLOT codes and unknown VDU 23 codes, which are routed to the unknown PLOT codes vector — see [[os/vdu-internals]]).
