...
sources: [naug-ch13-video, beebwiki-crtc]
sheila: ["&FE00", "&FE01"]
...
| 6-5 | BLK — cursor enable + blink rate (4-state field) |

**BLK encoding** (per `[[sources/beebwiki-crtc]]` — matches the 6845S datasheet):

| BLK | Effect |
|---|---|
| `00` | Cursor **off** |
| `01` | Cursor **on, no blink** (steady) |
| `10` | Blink slow — 1/16 field rate (~3 Hz at 50 Hz) |
| `11` | Blink fast — 1/32 field rate (~1.5 Hz; used during screen editing) |

(MOS sets BLK = `01` (steady-on) by default in all standard MODEs.)
...
## Chip variant — 6845S required

The BBC needs a 6845**S** specifically. Three S-only features are exercised:

1. **R3 vertical-sync width** field (bits 7-4) — earlier 6845s had only horizontal sync width.
2. **R6 odd values** — needed for `R6=25` in MODES 3/6/7.
3. **R8 character / cursor delay** (bits 4-7) — MODE 7 uses display delay = 1 and cursor delay = 2 to align with the SAA 5050's pipeline.
